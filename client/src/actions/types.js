// Alert
export const SET_ALERT = "SET_ALERT";
export const REMOVE_ALERT = "REMOVE_ALERT";
export const REMOVE_ALL_ALERTS = "REMOVE_ALL_ALERTS";

// Auth
export const REGISTER_SUCCESS = "REGISTER_SUCCESS";
export const REGISTER_FAIL = "REGISTER_FAIL";
export const USER_LOADED = "USER_LOADED";
export const AUTH_ERROR = "AUTH_ERROR";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAIL = "LOGIN_FAIL";
export const LOGOUT = "LOGOUT";
export const CLEAR_ERRORS = "CLEAR_ERRORS";

// Ad
export const GET_MY_ADS = " GET_MY_ADS";
export const POST_AD = "POST_AD";
export const UPDATE_AD = "UPDATE_AD";
export const DELETE_AD = "DELETE_AD";
export const SET_CURRENT = "SET_CURRENT";
export const CLEAR_CURRENT = "CLEAR_CURRENT";
export const SEARCH_ADS = "FILTER_ADS";
export const CLEAR_MY_ADS = "CLEAR_MY_ADS";
export const CLEAR_FILTER = "CLEAR_FILTER";
export const AD_ERROR = "AD_ERROR";
export const CLEAR_AD_ERROR = "CLEAR_AD_ERROR";
export const UPLOAD_IMAGES = "UPLOAD_IMAGES";
export const GET_AD_DETAILS = "GET_AD_DETAILS";
export const CLEAR_AD_DETAILS = "CLEAR_AD_DETAILS";
export const GET_AD_GROUP = "GET_AD_GROUP";
export const SET_AD_GROUP = "SET_AD_GROUP";

// General
export const SET_CURRENT_PAGE = "SET_CURRENT_PAGE";
export const REMOVE_CURRENT_PAGE = "REMOVE_CURRENT_PAGE";
export const VALIDATE_POSTCODE = "VALIDATE_POSTCODE";
export const POSTCODE_VALIDATION_ERROR = "POSTCODE_VALIDATION_ERROR";

// User
export const SAVE_AD = "SAVE_AD";
export const GET_SAVED_ADS_DETAILS = "GET_SAVED_ADS_DETAILS";
export const GET_SAVED_ADS_LIST = "GET_SAVED_ADS_LIST";
export const REMOVE_AD_FROM_SAVED = "REMOVE_AD_FROM_SAVED";
export const SAVE_AD_ERROR = "SAVE_AD_ERROR";
